...
export const Validators: Record<string, ValidatorFunction> = {
  ...
  address: (value) => {
    ...
    if (!trimmed) {
      ...
      result.warning = "This address is quite long. Please ensure it is accurate and complete.";
    }
  ...
  email: (value) => {
    ...
    } else if (!value.endsWith("@gmail.com") && !value.endsWith("@yahoo.com") && !value.endsWith("@outlook.com")) {
      result.warning = "Consider using your company email for business accounts.";
    }
  ...
  password: (value) => {
    ...
    if (!value) {
      ...
      result.error = "8-20 characters with uppercase, lowercase, number, and special character.";
    } else if (value.length === 8) {
      result.warning = "For better security, consider a longer password (12+ characters).";
    }
  ...
  note: (value) => {
    ...
    if (typeof value === "string" && value.length !== 0 && (value.length < 5 || value.length > 255)) {
